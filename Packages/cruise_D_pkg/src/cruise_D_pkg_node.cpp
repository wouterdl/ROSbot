...
{
    ros::init(argc, argv,"action_controller_D");
    ros::NodeHandle n("~");
    ros::Subscriber distfl_sub = n.subscribe("/range/fl_D", 1, distfl_callback);
    ros::Subscriber distfr_sub = n.subscribe("/range/fr_D", 1, distfr_callback);
    ros::Subscriber pose_sub = n.subscribe("/pose_D", 2, pose_callback);
    ros::Subscriber hough_sub = n.subscribe("/Hough_D", 1, hough_callback);


    ros::Rate loop_rate(10);
    clean_pub = n.advertise<geometry_msgs::Twist>("/cmd_vel_D", 1);
    set_vel.linear.x = 0;
    set_vel.linear.y = 0;
    set_vel.linear.z = 0;
    set_vel.angular.x = 0;
