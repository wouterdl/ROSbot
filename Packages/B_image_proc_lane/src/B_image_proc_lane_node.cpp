...
int main(int argc, char **argv)
{
  ...
  //Initializing Node
	ros::init(argc, argv, "image_proc_lane_B");
	ros::NodeHandle n;
	Lanes lanes;
	...
	ros::Publisher Hough_pub = n.advertise<std_msgs::Int64MultiArray>("Hough_B", 0);


	//Subscriber
	image_transport::ImageTransport it(n);
	image_transport::Subscriber sub;
	sub = it.subscribe("/camera_B/rgb/image_raw", 1, &Lanes::imagecallback, &lanes);

	image_transport::Publisher imgpub = it.advertise("edge_img_B", 1);
